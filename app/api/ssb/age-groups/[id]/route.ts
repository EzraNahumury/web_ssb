...
import { deleteAgeGroup, updateAgeGroup } from "@/lib/data";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const updated = await updateAgeGroup(routeData.id, routeData.ssbId, parsed.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  const deleted = await deleteAgeGroup(routeData.id, routeData.ssbId);
